...
GiD_ElementType giveTheTypeOfElementForGiD(const char *elementName) {
	...
	switch (i) {
	case 3: return GiD_Triangle;
	case 4: return GiD_Quadrilateral;
	case 5: return GiD_Tetrahedra;
	case 6: return GiD_Hexahedra;
	case 7: return GiD_Prism;
	default: throw ("Cannot find the type of element (Mesh)");
	}
...
void write_to_post_gid_file(Str_Mesh &pmesh){
	GiD_BeginMesh(pmesh.mesh_name_.c_str(), pmesh.ndim_ == 3 ? GiD_3D : GiD_2D,
			giveTheTypeOfElementForGiD(pmesh.element_name_.c_str()), pmesh.nnode_);
