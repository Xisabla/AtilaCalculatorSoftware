...
void Str_binary_data_GiD::write_one_step_to_post_gid_file(const float &step, const int &number_frame){
			...
			if (std::string::npos != result->results_.find("Real")) {
				...
				case 1: {
					...
					for (auto i = 0; i < number_frame; ++i) {
						...
						GiD_BeginResult(my_real->analysis_.c_str(), "Animate", i + 1,
								GiD_Scalar, GiD_OnNodes, NULL, NULL, 0, NULL/*&component_names*/);
						for (auto j = 0; j < my_real->number_of_results_; ++j) {
							...
							if (*data_real != GP_UNKNOWN) {
								auto square_dispx = (float)sqrt(*data_real* *data_real + *data_imag * *data_imag);
								GiD_WriteScalar(node_number_real, square_dispx * sin(atan2(*data_imag, *data_real) + theta));
							} else {
				...
				case 4: {
					...
					for (auto i = 0; i < number_frame; ++i) {
						auto theta = -2.0 * M_PI * (double)i / (double)(number_frame);
						GiD_BeginResult(my_real->analysis_.c_str(), "Animate", i + 1,
								GiD_Vector, GiD_OnNodes, NULL, NULL, 0, NULL /*component_names*/);
						for (auto j = 0; j < my_real->number_of_results_; ++j) {
							auto [node_number_real, data_real] = my_real->get_one_result(j);
							auto [node_number_imag, data_imag] = my_imag->get_one_result(j);
							if (node_number_real != node_number_imag){
							...
							if (*data_real != GP_UNKNOWN) {
								...
								auto square_dispx = (float)sqrt(results1_r*results1_r + results1_i*results1_i);
								auto square_dispy = (float)sqrt(results2_r*results2_r + results2_i*results2_i);
								auto square_dispz = (float)sqrt(results3_r*results3_r + results3_i*results3_i);
								...
								GiD_WriteVectorModule(node_number_real,GP_UNKNOWN, GP_UNKNOWN, GP_UNKNOWN, GP_UNKNOWN);
						}
						GiD_EndResult();
					}
				}
				...
				if (std::string::npos == result->results_.find("Magn")) {
					...
					case 1: {
						//				const char *component_names = result->component_names_[0].c_str();
						auto number_frame = 20;
						for (auto i = 0; i < number_frame; ++i) {
							auto theta = -2.0 * M_PI * (double)i / (double)(number_frame);
							auto stheta = sin(theta);
							GiD_BeginResult(result->analysis_.c_str(), "Animate", i + 1,
									GiD_Scalar, GiD_OnNodes, NULL, NULL, 0, NULL/* 1, &component_names*/);
						...
						for (auto i = 0; i < number_frame; ++i) {
							...
							for (auto j = 0; j < result->number_of_results_; ++j) {
								...
								if (*data_real != GP_UNKNOWN) {
									...
									auto results2_r = data_real[1];
									auto results3_r = data_real[2];
									GiD_WriteVector(node_number_real, results1_r*stheta, results2_r*stheta, results3_r*stheta);
